...
//munprotect system call makes page table entries both readable and writable
int
  ...
  for (i = (int) addr; i < ((int) addr + (len) *PGSIZE); i+= PGSIZE){
    ...
    */
    ...
    /*
      ...
      - PTE_W : Controls if the instruction is allowed to write to the page or not, 
...
/*
  ...
  When the hardware sees that you overwrote CR3 (even with the same value), it guarantees that your PTE updates will be used upon subsequent accesses. The lcr3() function will help you in this pursuit.
  lcr3(pagedirectory_t page)
  to flush the TLB after a changing a valid page table entry that might be cached in the TLB.
  One way to do this is by reloading the page table base register CR3 using:
  lcr3(V2P(myproc()->pgdir));
  V2P converts virtual address to physical address
  */
